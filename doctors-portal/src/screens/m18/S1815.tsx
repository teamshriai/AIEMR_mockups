  ...
  return (
    <Screen
      ...
      chips={
        <>
          {aiActive && (
            <Chip tone="ai" className="tabular">
              <Diamond size={9} />
              model {modelScore}
            </Chip>
          )}
          <Chip tone="brand" className="tabular">
            yours {humanScore}
          </Chip>
          {disagreements.length > 0 && (
            <Chip tone="caution">
              {disagreements.length} region{disagreements.length === 1 ? '' : 's'} changed
            </Chip>
          )}
        </>
      }
      ...
      rail={
        <div className="space-y-4">
          <Card className="p-4">
            <h3 className="text-[0.82em] font-semibold tracking-wide text-ink-3 uppercase">Both scores are kept</h3>
            <dl className="mt-2 divide-y divide-glass-hairline">
              {aiActive && (
                <KeyValue label="Model">
                  <span className="tabular font-semibold">{modelScore}/10</span>
                </KeyValue>
              )}
          ...
          <Card className="p-4">
            <h3 className="text-[0.82em] font-semibold tracking-wide text-ink-3 uppercase">What ASPECTS decides</h3>
            ...
            <p className="mt-2 text-[0.86em] text-ink-3">
              One point is one region. At the margin, one region changes the decision.
            </p>
          </Card>
        </div>
      }
    ...
    >
      <div className="space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
          ...
          <Card className="overflow-hidden">
            <div className="aspect-square w-full bg-[#0a0d14] p-4">
              <svg viewBox="0 0 200 200" className="size-full" role="img" aria-label="ASPECTS region map, left hemisphere">
            ...
            <p className="px-4 py-3 text-[0.86em] text-ink-3">
              A filled region is scored as affected. A dashed amber outline marks where you and the model disagree.
            </p>
          </Card>

          {/* The region list. */}
          <ScreenSection title="Ten regions, one point each">
            <Card className="overflow-hidden">
              <ul className="divide-y divide-glass-hairline">
                {ASPECTS_REGIONS.map((r) => {
                  ...
                  return (
                    <li
                      key={r.key}
                      className={cx('flex flex-wrap items-center justify-between gap-2 px-4 py-2.5', changed && 'bg-caution-soft/40')}
                    >
                      ...
                      <span className="flex items-center gap-1.5">
                        <Button
                          size="sm"
                          tone={affected ? 'secondary' : 'primary'}
                          onClick={() => setAspectsRegion(r.key, false)}
                        >
                          Normal
                        </Button>
                        <Button
                          size="sm"
                          tone={affected ? 'destructive' : 'secondary'}
                          onClick={() => setAspectsRegion(r.key, true)}
                        >
                          Affected
                        </Button>
                      </span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-t border-glass-hairline bg-glass-fill-strong px-4 py-3">
        ...
        {aiActive && (
          <Card className="p-4">
            <p className="flex flex-wrap items-center gap-2 text-[0.88em] font-semibold text-ink-3">
              <Diamond size={10} />
            ...
            <p className="mt-1.5 text-[0.9em] text-ink-2">
              ...
              subtle, and it does not distinguish an old infarct from a new one — which is the commonest reason a human
              overrules it. Manual region-by-region scoring is the fallback.
            </p>
          </Card>
        )}
